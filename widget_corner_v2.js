(function () {
    ...
    const script = document.currentScript || document.querySelector('script[data-n7-widget]');

    if (!script) {
        console.error("N7 Widget: script-тег не найден");
        return;
    }

    const CONFIG = {
        backendPort: script.dataset.backendPort,
        project: script.dataset.project,
        logoUrl: script.dataset.logoUrl,

        whatsapp: script.dataset.whatsapp,
        telegram: script.dataset.telegram,
        max: script.dataset.max,

        buttonColor: script.dataset.buttonColor,
        iconColor: script.dataset.iconColor,
        widgetColor: script.dataset.widgetColor
    }
    ...
    if (!PROJECTS || !LOGO_URL || !BACKEND_PORT) {
        console.error("N7 Widget: в конфигурации есть незаполненные поля", CONFIG);
        return;
    }
